"""Skill installation and management for Rxscientist.

This module provides functions for installing, listing, and uninstalling user skills.
Skills are installed to GLOBAL_SKILLS_DIR by default (~/.Rxscientist/skills/).
...
Usage:
    from Rxscientist.tools.skills_manager import install_skill, list_skills, uninstall_skill
...
def install_skill(
    ...
    Args:
        ...
        global_install: If True (default), install to GLOBAL_SKILLS_DIR
            (~/.Rxscientist/skills/). If False, install to the
...
def _install_from_github(source: str, dest_dir: str) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="Rxscientist-skill-") as tmp:
...
def list_skills(include_system: bool = False) -> list[SkillInfo]:
    ...
    # Tier 2: global skills (~/.Rxscientist/skills/)
...
def fetch_remote_skill_index(
    ...
    index: list[dict] = []
    with tempfile.TemporaryDirectory(prefix="Rxscientist-browse-") as tmp:
        clone_dir = os.path.join(tmp, "repo")
        _clone_repo(repo, ref, clone_dir)
