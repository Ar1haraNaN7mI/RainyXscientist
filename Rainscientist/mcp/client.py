...
def load_mcp_config() -> dict[str, Any]:
    ...
    Reads ``~/.config/Rxscientist/mcp.yaml`` and interpolates ``${VAR}``
...
def _route_tools(
    ...
    Returns:
        ...
        main Rxscientist agent; other keys match subagent names.
    """
...
async def aload_mcp_tools(config: dict[str, Any] | None = None) -> dict[str, list]:
    ...
    Args:
        config: Optional pre-loaded MCP config dict.  When ``None``,
            loads from ``~/.config/Rxscientist/mcp.yaml``.
    """
    if config is None:
...
def load_mcp_tools(config: dict[str, Any] | None = None) -> dict[str, list]:
    ...
    1. Loads user config from ``~/.config/Rxscientist/mcp.yaml``
    ...
    Args:
        config: Optional pre-loaded MCP config dict.  When ``None``,
            loads from ``~/.config/Rxscientist/mcp.yaml``.  Passing a
