...
```python
from Rxscientist.middleware import EvoMemoryMiddleware
...
EXTRACTION_PROMPT = """\
You are a memory extraction assistant for a scientific experiment agent called Rxscientist.
...
DEFAULT_MEMORY_TEMPLATE = """# Rxscientist Memory
...
def _ensure_memory_template(existing_md: str) -> str:
    ...
    result = existing_md
    if "# Rxscientist Memory" not in result:
        result = "# Rxscientist Memory\n\n" + result.lstrip()
