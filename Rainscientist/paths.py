"""Path resolution utilities for Rxscientist runtime directories."""
...
WORKSPACE_ROOT = _env_path("Rxscientist_WORKSPACE_DIR") or Path.cwd()

RUNS_DIR = _env_path("Rxscientist_RUNS_DIR") or (WORKSPACE_ROOT / "runs")
...
def _global_data_dir() -> Path:
    """Global application data directory (~/.Rxscientist/ by default).
    ...
    """
    return Path.home() / ".Rxscientist"


# Global data dir: ~/.Rxscientist/ by default, overridable via env var.
DATA_DIR: Path = _env_path("Rxscientist_DATA_DIR") or _global_data_dir()
...
# Global skills: shared across all workspaces (~/.Rxscientist/skills/)
GLOBAL_SKILLS_DIR: Path = _global_skills_dir()

# Global memories: shared across all workspaces (~/.Rxscientist/memories/)
GLOBAL_MEMORIES_DIR: Path = _global_memories_dir()

# Memories dir: global by default, overridable via env var.
# Supports both new (Rxscientist_MEMORIES_DIR) and old (Rxscientist_MEMORY_DIR) env vars.
MEMORIES_DIR: Path = (
    _env_path("Rxscientist_MEMORIES_DIR")
    or _env_path("Rxscientist_MEMORY_DIR")
    or GLOBAL_MEMORIES_DIR
)
MEMORY_DIR = MEMORIES_DIR  # backward compat alias

# Default GitHub owner/repo for remote skill index and MCP marketplace YAML
...
def migrate_legacy_sessions_db() -> None:
    ...
    ~/.config/Rxscientist/ to ~/.Rxscientist/.
    ...
    legacy = (
        (Path(xdg) / "Rxscientist")
        if xdg
        else (Path.home() / ".config" / "Rxscientist")
    )
