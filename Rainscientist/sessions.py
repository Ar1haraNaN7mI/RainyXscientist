...
def get_db_path() -> Path:
    ...
    Uses ``paths.DATA_DIR`` (~/.Rxscientist/ by default), then applies
...
async def list_threads(
    ...
    """List Rxscientist threads, most-recent first.
...
async def get_most_recent() -> str | None:
    """Return the most recent Rxscientist thread ID, or ``None``."""
    db_path = str(get_db_path())
    async with aiosqlite.connect(db_path, timeout=30.0) as conn:
        if not await _table_exists(conn, "checkpoints"):
            return None
...
async def thread_exists(thread_id: str) -> bool:
    """Return ``True`` if *thread_id* has at least one Rxscientist checkpoint."""
    db_path = str(get_db_path())
    async with aiosqlite.connect(db_path, timeout=30.0) as conn:
        if not await _table_exists(conn, "checkpoints"):
            return False
...
async def find_similar_threads(thread_id: str, limit: int = 5) -> list[str]:
    """Find Rxscientist thread IDs that start with *thread_id* (prefix match)."""
    db_path = str(get_db_path())
    async with aiosqlite.connect(db_path, timeout=30.0) as conn:
        if not await _table_exists(conn, "checkpoints"):
            return []
...
async def delete_thread(thread_id: str) -> bool:
    """Delete all Rxscientist checkpoints (and writes) for *thread_id*."""
    db_path = str(get_db_path())
    async with aiosqlite.connect(db_path, timeout=30.0) as conn:
        if not await _table_exists(conn, "checkpoints"):
            return False
...
async def get_thread_messages(thread_id: str) -> list:
    ...
    Only returns messages for Rxscientist threads.
    ...
    async with aiosqlite.connect(db_path, timeout=30.0) as conn:
        if not await _table_exists(conn, "checkpoints"):
            return []
        # Verify this thread belongs to Rxscientist before loading messages
        check = """
            SELECT 1 FROM checkpoints
            WHERE thread_id = ? AND json_extract(metadata, '$.agent_name') = ?
            LIMIT 1
