...
def onboard(
    ...
    """Interactive setup wizard for Rxscientist
...
def serve(
    ...
    """Run Rxscientist in headless mode -- channels only, no interactive prompt.
    ...
    if debug:
        os.environ["Rxscientist_LOG_LEVEL"] = "DEBUG"
        os.environ["Rxscientist_CHANNEL_DEBUG_TRACING"] = "true"
    ...
    if not config.channel_enabled:
        ...
        console.print("[dim]Run [bold]rxsci channel setup[/bold] first.[/dim]")
        raise typer.Exit(1)
...
def config_list():
    ...
    table = Table(title="Rxscientist Configuration", show_header=True)
...
def mcp_config(
    ...
    \b
    Examples:
      rxsci mcp config             # Show all servers in detail
      rxsci mcp config filesystem  # Show one server
    """
    status = _show_mcp_config(name or "", show_blank_line=False)
    if status == "empty":
        console.print(
            "[dim]Add one with:[/dim] rxsci mcp add <name> <transport> <command-or-url> [args...]"
        )
        return
    if status == "missing":
        raise typer.Exit(1)
...
def mcp_add(
    ...
    Examples:
      ...
      rxsci mcp add my-sse https://example.com/sse --transport sse -e research-agent
      rxsci mcp add brave-search npx --env-ref BRAVE_API_KEY -- -y @modelcontextprotocol/server-brave-search
    """
...
def mcp_edit(
    ...
    Examples:
      ...
      rxsci mcp edit filesystem -t read_file,write_file
      rxsci mcp edit my-api --url http://new-host:9090/mcp
      rxsci mcp edit my-api --tools none
    """
...
def mcp_install(
    ...
    \b
    Examples:
      rxsci mcp install                       # Interactive browser
      rxsci mcp install search                # Filter by 'search' tag
      rxsci mcp install sequential-thinking   # Install by name
    """
...
def _version_callback(value: bool):
    if value:
        typer.echo(f"Rxscientist {_pkg_version('Rxscientist')}")
        raise typer.Exit()
...
def _main_callback(
    ...
    """Rxscientist Agent - AI-powered research & code execution CLI"""
    ...
    enabled_channels = {
        item.strip().lower()
        for item in (config.channel_enabled or "").split(",")
        if item.strip()
    }
    if "mobile" not in enabled_channels:
