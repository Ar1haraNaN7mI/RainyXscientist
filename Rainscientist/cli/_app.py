...
Examples:
  ...
  rxsci mcp add local-server python -- /path/to/server.py
  ...
  rxsci mcp add my-server node --env API_KEY=xxx -- server.js
  ...
  rxsci mcp add local-server python -e main,research-agent,code-agent -- /path/to/server.py

  # Explicit transport override:
  rxsci mcp add my-sse https://example.com/sse --transport sse
