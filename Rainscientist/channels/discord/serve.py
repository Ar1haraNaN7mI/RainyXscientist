...
Usage:
    python -m Rxscientist.channels.discord.serve --bot-token TOKEN [OPTIONS]

Examples:
    # Allow all senders (default)
    python -m Rxscientist.channels.discord.serve --bot-token TOKEN

    # Only allow specific senders and channels
    python -m Rxscientist.channels.discord.serve --bot-token TOKEN --allow 123 --allow-channel 456

    # With proxy, agent and thinking
    python -m Rxscientist.channels.discord.serve --bot-token TOKEN --proxy http://proxy:8080 --agent --thinking
"""
