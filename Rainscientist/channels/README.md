...
Rxscientist provides unified integration with 10 messaging platforms. This document covers the architecture overview, message processing pipeline, capability matrix, security model, deployment guides, and troubleshooting.

Configuration file: `~/.config/Rxscientist/config.yaml` (or use environment variables with the `Rxscientist_` prefix).
...
```bash
pip install Rxscientist[telegram]
...
rxsci config set channel_enabled telegram
rxsci config set telegram_bot_token "123456:ABC-xxx"

# Option C: Environment variables (Rxscientist_ prefix, uppercase)
export Rxscientist_CHANNEL_ENABLED=telegram
export Rxscientist_TELEGRAM_BOT_TOKEN="123456:ABC-xxx"
```

### 3. Start

```bash
rxsci serve                # Start agent + all enabled channels
# or
rxsci channel start        # Standalone channel mode (message loop only)
```
...
**Install:** `pip install Rxscientist[telegram]`
...
**Env vars:** `Rxscientist_TELEGRAM_BOT_TOKEN`, `Rxscientist_TELEGRAM_ALLOWED_SENDERS`, `Rxscientist_TELEGRAM_PROXY`
...
**Install:** `pip install Rxscientist[discord]`
...
**Env vars:** `Rxscientist_DISCORD_BOT_TOKEN`, `Rxscientist_DISCORD_ALLOWED_SENDERS`, `Rxscientist_DISCORD_ALLOWED_CHANNELS`, `Rxscientist_DISCORD_PROXY`
...
**Install:** `pip install Rxscientist[slack]`
...
**Env vars:** `Rxscientist_SLACK_BOT_TOKEN`, `Rxscientist_SLACK_APP_TOKEN`, `Rxscientist_SLACK_ALLOWED_SENDERS`, `Rxscientist_SLACK_ALLOWED_CHANNELS`, `Rxscientist_SLACK_PROXY`
...
**Install:** `pip install Rxscientist[feishu]`
...
```bash
# Install the SDK
pip install 'Rxscientist[feishu]'

# Via config file
feishu_subscription_mode: "websocket"

# Via CLI (standalone)
python -m Rxscientist.channels.feishu.serve --app-id ID --app-secret SECRET --mode websocket
```
...
**Env vars:** `Rxscientist_FEISHU_APP_ID`, `Rxscientist_FEISHU_APP_SECRET`, `Rxscientist_FEISHU_WEBHOOK_PORT`, `Rxscientist_FEISHU_DOMAIN`
...
**Install:** `pip install Rxscientist[wechat]`
...
**Install:** `pip install Rxscientist[dingtalk]`
...
**Env vars:** `Rxscientist_DINGTALK_CLIENT_ID`, `Rxscientist_DINGTALK_CLIENT_SECRET`
...
**Install:** `pip install Rxscientist[qq]`
...
**Env vars:** `Rxscientist_QQ_APP_ID`, `Rxscientist_QQ_APP_SECRET`

**Technical details:** Uses `qq-botpy` SDK via WebSocket to connect to QQ Bot Gateway. Supports C2C (direct) and group messages. Outbound replies prefer native QQ markdown messages (`msg_type=2`) so headings/lists/code fences keep their structure; when the SDK/API rejects markdown, Rxscientist falls back to plain text with Markdown stripped but line structure preserved. Message deduplication (1000-entry LRU cache). Group @mention filtering (strips first `@bot`). Intents: `public_messages=True`, `direct_message=True`. Text chunk limit: 4096 chars.

---

### Signal

**Install:** `pip install Rxscientist[signal]` (also requires [signal-cli](https://github.com/AsamK/signal-cli) installed separately)
...
2. Register or link a phone number:
   ...
   - Link existing device: `signal-cli link -n "Rxscientist"`
3. Rxscientist will auto-start the signal-cli daemon if it's not already running.
...
**Env vars:** `Rxscientist_SIGNAL_PHONE_NUMBER`, `Rxscientist_SIGNAL_CLI_PATH`, `Rxscientist_SIGNAL_RPC_PORT`
...
**Install:** `pip install Rxscientist[email]` (core dependencies included, no extras needed)
...
**Env vars:** `Rxscientist_EMAIL_IMAP_HOST`, `Rxscientist_EMAIL_IMAP_USERNAME`, `Rxscientist_EMAIL_IMAP_PASSWORD`, `Rxscientist_EMAIL_SMTP_HOST`, `Rxscientist_EMAIL_SMTP_USERNAME`, `Rxscientist_EMAIL_SMTP_PASSWORD`
...
**Env vars:** `Rxscientist_IMESSAGE_CLI_PATH`, `Rxscientist_IMESSAGE_SERVICE`, `Rxscientist_IMESSAGE_ALLOWED_SENDERS`
...
WORKDIR /app
COPY . .
RUN pip install Rxscientist[feishu,wechat]
...
```bash
docker build -t Rxscientist .
docker run -d \
  -p 9000:9000 \
  -p 9001:9001 \
  -e Rxscientist_CHANNEL_ENABLED="feishu,wechat" \
  -e Rxscientist_FEISHU_APP_ID="cli_xxx" \
  -e Rxscientist_FEISHU_APP_SECRET="xxx" \
  -e Rxscientist_WECHAT_BACKEND="wecom" \
  -e Rxscientist_WECHAT_WECOM_CORP_ID="ww..." \
  -e Rxscientist_WECHAT_WECOM_SECRET="xxx" \
  Rxscientist
```
...
```bash
docker run -d \
  -e Rxscientist_CHANNEL_ENABLED="telegram" \
  -e Rxscientist_TELEGRAM_BOT_TOKEN="123456:ABC-xxx" \
  Rxscientist
```
...
2. Verify the bot token/credentials are correct: `rxsci config get telegram_bot_token`
...
```bash
pip install Rxscientist[telegram]  # or discord, slack, feishu, etc.
```
...
```bash
export Rxscientist_LOG_LEVEL=DEBUG
# or
rxsci config set log_level debug
```

Channel-specific log output is prefixed with the module path (e.g., `Rxscientist.channels.telegram.channel`).
