"""Rxscientist Agent graph construction.
...
(``rxsci config list``, ``rxsci onboard``) never pay the cost.
...
def _get_default_agent():
    ...
    global _Rxscientist_agent
    if _Rxscientist_agent is None:
        from deepagents import create_deep_agent

        be = _get_default_backend()
        mw = _get_default_middleware()
        kwargs = load_mcp_and_build_kwargs(be, mw)
        _Rxscientist_agent = create_deep_agent(**kwargs).with_config(
            {"recursion_limit": 1000}
        )
    return _Rxscientist_agent


def __getattr__(name: str):
    if name == "Rxscientist_agent":
...
def create_cli_agent(workspace_dir: str | None = None, checkpointer=None, config=None):
    ...
    Args:
        ...
        config: Optional pre-loaded ``RxscientistConfig``.  If ``None``,
