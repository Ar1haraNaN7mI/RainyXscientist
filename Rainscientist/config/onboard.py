"""Interactive onboarding wizard for Rxscientist.
...
def _print_header() -> None:
    ...
    console.print(
        Panel.fit(
            Text.from_markup(
                "[bold cyan]Rxscientist Setup Wizard[/bold cyan]\n\n"
                "This wizard will help you configure Rxscientist.\n"
...
def _step_ui_backend(config: RxscientistConfig) -> str:
...
def _step_provider(config: RxscientistConfig) -> str:
...
def _step_minimax_region(config: RxscientistConfig) -> str:
...
def _prompt_ccproxy_port(config: RxscientistConfig) -> None:
...
def _step_anthropic_auth_mode(config: RxscientistConfig) -> str:
    ...
    choices = [
        ...
        Choice(
            ...
            + (
                ...
                else " [requires: pip install Rxscientist[oauth]]"
            ),
            value="oauth",
        ),
    ]
    ...
    if auth_mode == "oauth" and not ccproxy_available:
        ...
        install = questionary.confirm(
            'Install ccproxy now? (pip install "Rxscientist[oauth]")',
            default=True,
            style=WIZARD_STYLE,
            qmark=f"  {QMARK}",
        ).ask()
        ...
        if install:
            ...
            console.print(
                '  [dim]Skipped. Install manually: pip install "Rxscientist[oauth]"[/dim]'
            )
            return "api_key"
...
def _step_base_url(config: RxscientistConfig, current_value: str | None = None) -> str:
...
def _step_ollama_base_url(config: RxscientistConfig) -> tuple[str, list[str]]:
...
def _step_reasoning_effort(config: RxscientistConfig) -> str:
...
def _step_workspace(config: RxscientistConfig) -> str:
...
def _step_thinking(config: RxscientistConfig) -> bool:
...
def _step_skills() -> list[str]:
    ...
    if not selected:
        ...
        if has_npx:
            ...
            console.print(
                "  [yellow bold]* Less is more[/yellow bold] [dim](Rxscientist can discover and install skills on its own)[/dim]"
            )
        else:
...
def _step_mcp_servers() -> list[str]:
    ...
    if not selected:
        ...
        console.print(
            "  [dim]Add later with: rxsci mcp add <name> <command> [--env-ref KEY] -- [args][/dim]"
        )
        return []
...
def _install_ccproxy() -> bool:
    """Run pip install for ccproxy (Rxscientist[oauth]).
    ...
    ok = install_library("Rxscientist[oauth]")
    if not ok:
        console.print("  [red]✗ Installation failed.[/red]")
        return False
...
def _step_channels(config: RxscientistConfig) -> dict[str, object]:
    ...
    # exact dependency without requiring the Rxscientist package itself
            ...
            try:
                ...
                if install_now:
                    ...
                    if _pip_pkgs:
                        ...
                        _ok = install_library(f"Rxscientist[{pip_extra}]")
                    if _ok:
                        ...
                        try:
                            ...
                            console.print(
                                "  [dim]Try restarting and running:[/dim] rxsci channel setup"
                            )
                    else:
                        console.print("  [red]✗ Installation failed.[/red]")
                        console.print(
...
def render_progress(current_step: int, completed: set[int]) -> Panel:
    ...
    content = Text("\n").join(lines)
    return Panel(content, title="[bold]Rxscientist Setup[/bold]", border_style="blue")
