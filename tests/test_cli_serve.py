...
def _run_serve_once(
    ...
    *,
    ...
    import Rxscientist.config as config_mod
...
def test_serve_debug_sets_log_level_and_channel_trace(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("Rxscientist_LOG_LEVEL", raising=False)
    monkeypatch.delenv("Rxscientist_CHANNEL_DEBUG_TRACING", raising=False)
    ...
    monkeypatch.setattr(
        ...
        lambda: configure_calls.append(
            (
                os.environ.get("Rxscientist_LOG_LEVEL"),
                os.environ.get("Rxscientist_CHANNEL_DEBUG_TRACING"),
            )
        ),
    )
