...
from Rxscientist.channels.debug import (
    TraceMixin,
    debug_trace_enabled,
    emit_debug_event,
    emit_debug_event_if,
...
def _make_raw(**overrides):
    ...
    from Rxscientist.channels.base import RawIncoming
...
def test_middleware_dedup_emits_structured_event(caplog):
    from Rxscientist.channels.middleware import DedupMiddleware
...
def test_middleware_allowlist_emits_structured_event(caplog):
    from Rxscientist.channels.middleware import AllowListMiddleware
...
def test_middleware_mention_gating_emits_structured_event(caplog):
    from Rxscientist.channels.middleware import MentionGatingMiddleware
...
def test_typing_manager_emits_trace_events(caplog):
    from Rxscientist.channels.middleware import TypingManager
...
def test_ack_reaction_emits_error_traces(caplog):
    from Rxscientist.channels.middleware import AckReactionMiddleware
...
def test_inbound_raw_event_emitted(caplog):
    ...
    from Rxscientist.channels.base import Channel, RawIncoming
...
def test_standalone_dispatcher_treats_false_send_as_error(caplog):
    from Rxscientist.channels.bus import MessageBus
    from Rxscientist.channels.bus.events import OutboundMessage
    from Rxscientist.channels.standalone import standalone_outbound_dispatcher
...
def test_emit_debug_event_warns_on_level_mismatch(caplog):
    import Rxscientist.channels.debug as dbg
