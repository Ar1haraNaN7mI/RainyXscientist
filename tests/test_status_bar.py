...
def test_build_session_status_snapshot_uses_fallback_window(monkeypatch):
    ...
    monkeypatch.setattr(
        "Rxscientist.cli.status_bar.get_thread_messages",
        _fake_messages,
    )
    monkeypatch.setattr(
        "Rxscientist.cli.status_bar._get_default_chat_model",
        lambda: _FakeModel(),
    )
    monkeypatch.setattr(
        "Rxscientist.cli.status_bar.count_tokens_approximately",
        _fake_count,
    )
...
def test_apply_assistant_text_to_snapshot_updates_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "Rxscientist.cli.status_bar.estimate_message_tokens",
