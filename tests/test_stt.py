...
def test_transcribe_en_uses_whisper():
    import Rxscientist.stt as stt_mod
...
def test_enqueue_raw_stt_prepends_transcript():
    ...
    async def _run():
        with (
            patch(
                "Rxscientist.stt.transcribe_file", new=AsyncMock(return_value="你好")
            ),
            patch("Rxscientist.stt.is_audio_file", return_value=True),
        ):
...
def test_enqueue_raw_stt_disabled_skips_transcription():
    ...
    async def _run():
        with patch("Rxscientist.stt.transcribe_file", mock_transcribe):
            await ch._enqueue_raw(raw)
...
def test_enqueue_raw_stt_appends_to_existing_text():
    ...
    async def _run():
        with (
            patch(
                "Rxscientist.stt.transcribe_file",
...
#
# 1. Install deps:
#      uv pip install 'Rxscientist[stt]'
#
# 2. Enable STT:
#      rxsci config set stt_enabled true
#      rxsci config set stt_language zh   # zh / en / auto
#
# 3. Transcribe a local audio file directly:
#      python -c "
#      import asyncio
#      from Rxscientist.stt import transcribe_file
