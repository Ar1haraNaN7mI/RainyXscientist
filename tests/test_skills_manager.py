"""Tests for Rxscientist.tools.skills_manager module."""

from pathlib import Path
from unittest.mock import patch

import pytest
from Rxscientist.tools.skills_manager import (
    _is_github_url,
    _parse_github_url,
    _parse_skill_md,
    _validate_skill_dir,
...
def temp_skills_dir(tmp_path):
    ...
    empty_global.mkdir()
    with patch("Rxscientist.paths.GLOBAL_SKILLS_DIR", empty_global):
        yield skills_dir
...
class TestListSkills:
    ...
    def test_list_empty_dir(self, temp_skills_dir):
        with patch("Rxscientist.paths.USER_SKILLS_DIR", temp_skills_dir):
...
class TestFetchRemoteSkillIndex:
    ...
    def test_fetch_from_local_clone(self, tmp_path):
        ...
        with patch(
            "Rxscientist.tools.skills_manager._clone_repo", side_effect=fake_clone
        ):
            # Clear cache to ensure fresh fetch
            from Rxscientist.tools.skills_manager import _REMOTE_INDEX_CACHE
...
class TestSkillManagerList:
    ...
    def test_list_user_skills_workspace(self, tmp_path):
        ...
        from Rxscientist.tools.skill_manager import skill_manager
        ...
        with (
            patch("Rxscientist.paths.USER_SKILLS_DIR", workspace_dir),
            patch("Rxscientist.paths.GLOBAL_SKILLS_DIR", global_dir),
        ):
    ...
    def test_list_include_system_shows_both_sections(self, tmp_path):
        ...
        from Rxscientist.tools.skill_manager import skill_manager
        from Rxscientist.tools.skills_manager import SkillInfo
        ...
        with (
            ...
            patch("Rxscientist.paths.GLOBAL_SKILLS_DIR", global_dir),
            patch(
                "Rxscientist.tools.skills_manager.list_skills",
                return_value=[
                    SkillInfo(
                        name="user-skill",
