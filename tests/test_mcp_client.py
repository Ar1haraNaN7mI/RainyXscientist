"""Tests for Rxscientist.mcp module."""
...
def mcp_config_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("Rxscientist.mcp.client.USER_MCP_CONFIG", cfg)
    return cfg
...
class TestResolveCommand:
    ...
    def test_found_on_path(self):
        ...
        assert name.startswith(("python", "python3"))
    ...
    def test_found_in_python_bin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "Rxscientist.mcp.client.sys.executable", str(bin_dir / "python")
        )
    ...
    def test_not_found_returns_original(self, monkeypatch):
        ...
        monkeypatch.setattr("shutil.which", lambda _: None)
        monkeypatch.setattr(
            "Rxscientist.mcp.client.sys.executable", "/nonexistent/bin/python"
        )
    ...
    def test_build_connections_resolves_command(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "Rxscientist.mcp.client._resolve_command", lambda cmd: f"/resolved/{cmd}"
        )
...
def user_mcp_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("Rxscientist.mcp.client.USER_CONFIG_DIR", cfg_dir)
    monkeypatch.setattr("Rxscientist.mcp.client.USER_MCP_CONFIG", cfg_file)
    return cfg_file
...
class TestUvToolCompat:
    ...
    def test_is_uv_tool_env_false_when_no_virtual_env(self, monkeypatch):
        from Rxscientist.mcp.registry import _is_uv_tool_env
    ...
    def test_is_uv_tool_env_true_unix(self, monkeypatch):
        ...
        monkeypatch.setenv(
            "VIRTUAL_ENV", "/home/user/.local/share/uv/tools/Rxscientist"
        )
    ...
    def test_is_uv_tool_env_true_windows_backslashes(self, monkeypatch):
        ...
        monkeypatch.setenv(
            "VIRTUAL_ENV", r"C:\Users\user\AppData\Local\uv\tools\Rxscientist"
        )
        assert _is_uv_tool_env() is True
    ...
    def test_uv_tool_name_returns_name(self, monkeypatch):
        import Rxscientist.mcp.registry as reg

        monkeypatch.setenv(
            "VIRTUAL_ENV", "/home/user/.local/share/uv/tools/Rxscientist"
        )
        assert reg._uv_tool_name() == "Rxscientist"
    ...
    def test_pip_install_hint_uv_tool(self, monkeypatch):
        ...
        assert "uv tool install --reinstall Rxscientist --with" in hint
    ...
    def test_resolve_command_path_absolute_passthrough(self):
        from Rxscientist.mcp.registry import _resolve_command_path
