"""Tests for Rxscientist.paths — set_workspace_root and ensure_dirs."""
...
class TestSetWorkspaceRoot:
    ...
    def test_updates_derived_dirs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("Rxscientist_MEMORIES_DIR", raising=False)
        monkeypatch.delenv("Rxscientist_MEMORY_DIR", raising=False)
        monkeypatch.delenv("Rxscientist_RUNS_DIR", raising=False)
        monkeypatch.delenv("Rxscientist_SKILLS_DIR", raising=False)
    ...
    def test_preserves_env_overrides(self, tmp_path):
        ...
        env = {
            "Rxscientist_MEMORIES_DIR": str(custom_mem),
            "Rxscientist_SKILLS_DIR": str(custom_skills),
            "Rxscientist_RUNS_DIR": str(custom_runs),
        }
...
class TestEnsureDirsUsesUpdatedPaths:
    ...
    def test_ensure_dirs_uses_updated_paths(self, tmp_path):
        ...
        with mock.patch.dict(os.environ, {"Rxscientist_MEMORIES_DIR": str(custom_mem)}):
            paths.set_workspace_root(new_root)
            paths.ensure_dirs()
...
class TestLegacySessionsDbMigration:
    ...
    def _setup(self, tmp_path, monkeypatch):
        ...
        tmp_path/fake_home so legacy resolves to tmp_path/fake_home/.config/Rxscientist.
        ...
        fake_home.mkdir()
        legacy_dir = fake_home / ".config" / "Rxscientist"
