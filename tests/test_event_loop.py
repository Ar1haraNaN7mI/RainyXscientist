...
from Rxscientist.stream.display import _create_event_loop, _get_event_loop
...
class TestMultipleStreamingCalls:
    ...
    def test_sequential_streaming_calls(self):
        ...
        from Rxscientist.stream.display import _run_streaming
        ...
        with patch(
            "Rxscientist.stream.display.stream_agent_events", side_effect=mock_stream
        ):
            # Patch Live to avoid terminal output during tests
            with patch("Rxscientist.stream.display.Live"):
                # First call
                _run_streaming(
                    agent=mock_agent,
    ...
    def test_recursive_streaming_does_not_resend_same_thinking(self):
        ...
        with patch(
            "Rxscientist.stream.display.stream_agent_events",
