...
from Rxscientist.cli.tui_runtime import (
...
def test_resolve_ui_backend_falls_back_when_textual_unavailable(monkeypatch):
    monkeypatch.setattr(
        "Rxscientist.cli.tui_runtime._has_textual_support", lambda: False
    )
...
def test_resolve_ui_backend_keeps_tui_when_available(monkeypatch):
    monkeypatch.setattr(
        "Rxscientist.cli.tui_runtime._has_textual_support", lambda: True
    )
...
def test_run_streaming_falls_back_to_cli_on_runtime_error(monkeypatch):
    monkeypatch.setattr(
        "Rxscientist.cli.tui_runtime.get_backend", lambda *a, **k: _BrokenBackend()
    )
    ...
    monkeypatch.setattr(
        "Rxscientist.cli.tui_runtime.RichStreamingBackend", lambda: _RichStub()
    )
