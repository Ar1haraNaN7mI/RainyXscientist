"""Tests for Rxscientist onboarding wizard."""
...
class TestRenderProgress:
    ...
    def test_panel_has_title(self):
        ...
        panel = render_progress(current_step=0, completed=set())
        assert "Rxscientist Setup" in str(panel.title)
...
class TestStepProvider:
    def test_returns_selected_provider(self):
        ...
        from Rxscientist.config.onboard import _step_provider
...
class TestStepModel:
    def test_returns_selected_model(self):
        ...
        from Rxscientist.config.onboard import _step_model
...
class TestStepWorkspace:
    def test_returns_daemon_mode(self):
        ...
        from Rxscientist.config.onboard import _step_workspace
...
class TestPromptAndValidateApiKey:
    def test_keep_existing_key_still_validates(self):
        ...
        from Rxscientist.config.onboard import _prompt_and_validate_api_key

        validate_fn = Mock(return_value=(True, "Valid"))

        with (
            patch("Rxscientist.config.onboard.questionary") as mock_q,
            patch("Rxscientist.config.onboard.console"),
        ):
...
class TestValidateImessage:
    def test_valid_when_cli_found_with_rpc(self):
        ...
        from Rxscientist.config.onboard import validate_imessage

        version_result = Mock(returncode=0, stdout="imsg 1.2.3")
        rpc_result = Mock(returncode=0)

        with (
            patch("Rxscientist.config.onboard.sys") as mock_sys,
            patch("Rxscientist.channels.imessage.probe.shutil") as mock_shutil,
            patch("Rxscientist.config.onboard.subprocess") as mock_sub,
        ):
    ...
    def test_invalid_on_non_macos(self):
        ...
        with patch("Rxscientist.config.onboard.sys") as mock_sys:
...
class TestInstallImsg:
    def test_install_success(self):
        ...
        from Rxscientist.config.onboard import _install_imsg

        with patch("Rxscientist.config.onboard.subprocess") as mock_sub:
...
class TestSetupImessage:
    def test_already_installed(self):
        ...
        from Rxscientist.config.onboard import _setup_imessage

        with (
            patch(
                "Rxscientist.config.onboard.validate_imessage",
    ...
    def test_install_then_valid(self):
        ...
        with (
            patch("Rxscientist.config.onboard.validate_imessage") as mock_val,
            patch("Rxscientist.config.onboard._install_imsg", return_value=True),
            patch("Rxscientist.config.onboard.questionary") as mock_q,
            patch("Rxscientist.config.onboard.console"),
        ):
...
class TestStepSkills:
    def test_returns_empty_when_none_selected(self):
        ...
        from Rxscientist.config.onboard import _step_skills
    ...
    def test_installs_selected_skills(self):
        ...
        from Rxscientist.config.onboard import _RECOMMENDED_SKILLS, _step_skills
        ...
        with (
            patch("Rxscientist.config.onboard.questionary") as mock_q,
            patch("Rxscientist.config.onboard.console"),
            patch("Rxscientist.tools.skills_manager.install_skill") as mock_install,
        ):
...
class TestStepChannels:
    def test_returns_disabled_when_skip(self):
        ...
        from Rxscientist.config.onboard import _step_channels
    ...
    def test_returns_enabled_when_setup_passes(self):
        ...
        with (
            patch("Rxscientist.config.onboard.questionary") as mock_q,
            patch("Rxscientist.config.onboard._setup_imessage", return_value=True),
        ):
    ...
    def test_setup_fails_user_declines(self):
        ...
        with (
            patch("Rxscientist.config.onboard.questionary") as mock_q,
            patch("Rxscientist.config.onboard._setup_imessage", return_value=False),
        ):
            mock_q.checkbox.return_value.ask.return_value = ["imessage"]
            mock_q.confirm.return_value.ask.return_value = False
            result = _step_channels(config)
    ...
    def test_telegram_channel_selected(self):
        ...
        with (
            patch("Rxscientist.config.onboard.questionary") as mock_q,
            patch("Rxscientist.config.onboard._probe_channel"),
...
class TestStepMcpServersNpxFailure:
    def _make_test_servers(self):
        from Rxscientist.mcp.registry import MCPServerEntry
    ...
    def test_npx_failure_skips_npx_servers(self):
        ...
        from Rxscientist.config.onboard import _step_mcp_servers

        servers = self._make_test_servers()

        with (
            patch(
                "Rxscientist.mcp.registry.fetch_marketplace_index",
                return_value=servers,
            ),
            patch(
                "Rxscientist.config.onboard._checkbox_ask",
                return_value=["npx-server", "url-server"],
            ),
            patch("Rxscientist.config.onboard._ensure_npx", return_value=False),
            patch("Rxscientist.config.onboard._check_npx", return_value=False),
            patch("Rxscientist.mcp.client._load_user_config", return_value={}),
            patch("Rxscientist.mcp.client.add_mcp_server") as mock_add,
            patch("Rxscientist.config.onboard.console"),
        ):
    ...
    def test_npx_failure_returns_empty_when_all_npx(self):
        ...
        with (
            ...
            patch("Rxscientist.config.onboard._checkbox_ask", return_value=npx_names),
            patch("Rxscientist.config.onboard._ensure_npx", return_value=False),
            patch("Rxscientist.config.onboard._check_npx", return_value=False),
            patch("Rxscientist.mcp.client._load_user_config", return_value={}),
            patch("Rxscientist.mcp.client.add_mcp_server") as mock_add,
...
class TestStepThinking:
    def test_returns_show_thinking(self):
        ...
        from Rxscientist.config.onboard import _step_thinking
    ...
    def test_returns_false_when_off(self):
        ...
        config = RxscientistConfig(show_thinking=False)
...
class TestRunOnboard:
    def test_returns_true_on_save(self):
        ...
        from Rxscientist.config.onboard import run_onboard

        with (
            patch("Rxscientist.config.onboard.questionary") as mock_q,
            patch("Rxscientist.config.onboard.load_config") as mock_load,
            patch("Rxscientist.config.onboard.save_config") as mock_save,
            patch("Rxscientist.config.onboard.console"),
            patch("Rxscientist.config.onboard._step_tinytex"),
        ):
            # Setup mock config
            mock_load.return_value = RxscientistConfig()
...
class TestCheckLatexComponents:
    ...
    def test_all_available(self):
        ...
        from Rxscientist.config.onboard import _check_latex_components

        with (
            patch("Rxscientist.config.onboard.shutil") as mock_sh,
            patch("Rxscientist.config.onboard.subprocess") as mock_sp,
        ):
    ...
    def test_none_available(self):
        ...
        with patch("Rxscientist.config.onboard.shutil") as mock_sh:
...
class TestAutoInstallLatexmk:
    ...
    def test_success(self):
        ...
        from Rxscientist.config.onboard import _auto_install_latexmk

        with (
            patch("Rxscientist.config.onboard.shutil") as mock_sh,
            patch("Rxscientist.config.onboard.subprocess") as mock_sp,
            patch("Rxscientist.config.onboard.console") as mock_con,
        ):
...
class TestCheckTinytex:
    ...
    def test_found_pdflatex(self):
        ...
        from Rxscientist.config.onboard import _check_tinytex
...
class TestDetectTinytexInstallMethod:
    ...
    def test_macos_with_curl(self):
        ...
        from Rxscientist.config.onboard import _detect_tinytex_install_method
...
class TestInstallTinytex:
    ...
    def test_curl_install_success(self):
        ...
        from Rxscientist.config.onboard import _install_tinytex

        with patch("Rxscientist.config.onboard.subprocess") as mock_sp:
            mock_sp.run.return_value = Mock(returncode=0)
            mock_sp.TimeoutExpired = subprocess.TimeoutExpired
...
class TestStepTinytex:
    ...
    def test_user_declines_prepare(self):
        ...
        from Rxscientist.config.onboard import _step_tinytex

        with (
            patch("Rxscientist.config.onboard.questionary") as mock_q,
            patch("Rxscientist.config.onboard._print_step_skipped") as mock_ps,
            patch("Rxscientist.config.onboard.console"),
        ):
            mock_q.select.return_value.ask.return_value = False
            _step_tinytex()
    ...
    def test_already_installed_all_components(self):
        ...
        with (
            patch("Rxscientist.config.onboard.questionary") as mock_q,
            patch(
                "Rxscientist.config.onboard._check_latex_components",
                return_value={
                    "pdflatex": True,
                    "latexmk": True,
                    "tlmgr": True,
            ...
            patch("Rxscientist.config.onboard._print_latex_status") as mock_status,
            patch("Rxscientist.config.onboard.console"),
        ):
            mock_q.select.return_value.ask.return_value = True
            _step_tinytex()
    ...
    def test_already_installed_missing_latexmk(self):
        ...
        with (
            ...
            patch("Rxscientist.config.onboard._print_latex_status"),
            patch("Rxscientist.config.onboard._auto_install_latexmk") as mock_auto,
            patch("Rxscientist.config.onboard.console"),
        ):
            mock_q.select.return_value.ask.return_value = True
            _step_tinytex()
    ...
    def test_user_installs_successfully(self):
        ...
        with (
            ...
            patch(
                "Rxscientist.config.onboard._detect_tinytex_install_method",
                return_value=("curl", "curl ... | sh"),
            ),
            patch(
                "Rxscientist.config.onboard._install_tinytex",
                return_value=True,
            ),
            patch("Rxscientist.config.onboard.questionary") as mock_q,
            patch("Rxscientist.config.onboard._print_step_result") as mock_pr,
            patch("Rxscientist.config.onboard._print_latex_status"),
            patch("Rxscientist.config.onboard.console"),
        ):
            mock_q.select.return_value.ask.return_value = True
