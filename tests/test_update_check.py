"""Tests for Rxscientist.update_check module."""

import json
import time
from unittest.mock import MagicMock, patch

from Rxscientist.update_check import (
    CACHE_TTL,
    _parse_version,
    get_latest_version,
    is_update_available,
...
class TestGetLatestVersion:
    ...
    def test_fresh_cache_hit(self, tmp_path):
        ...
        with patch("Rxscientist.update_check.CACHE_FILE", cache_file):
    ...
    def test_stale_cache_fetches_pypi(self, tmp_path):
        ...
        with (
            patch("Rxscientist.update_check.CACHE_FILE", cache_file),
            patch("Rxscientist.update_check.CACHE_DIR", tmp_path),
...
class TestIsUpdateAvailable:
    ...
    def test_newer_version_available(self):
        with (
            patch("Rxscientist.update_check.get_latest_version", return_value="9.9.9"),
            patch("Rxscientist.update_check._installed_version", return_value="0.0.2"),
        ):
    ...
    def test_same_version(self):
        with (
            patch("Rxscientist.update_check.get_latest_version", return_value="0.0.2"),
            patch("Rxscientist.update_check._installed_version", return_value="0.0.2"),
        ):
    ...
    def test_older_pypi_version(self):
        with (
            patch("Rxscientist.update_check.get_latest_version", return_value="0.0.1"),
            patch("Rxscientist.update_check._installed_version", return_value="0.0.2"),
        ):
            available, _latest = is_update_available()
            assert available is False
    ...
    def test_pypi_unreachable(self):
        with patch("Rxscientist.update_check.get_latest_version", return_value=None):
            available, latest = is_update_available()
            assert available is False
    ...
    def test_invalid_version_string(self):
        with (
            patch("Rxscientist.update_check.get_latest_version", return_value="abc"),
            patch("Rxscientist.update_check._installed_version", return_value="0.0.2"),
        ):
            available, _latest = is_update_available()
            assert available is False
