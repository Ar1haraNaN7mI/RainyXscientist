"""Tests for Rxscientist configuration management."""
...
def temp_config_dir(tmp_path, monkeypatch):
    ...
    config_dir = tmp_path / "Rxscientist"
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    # Prevent load_dotenv from loading the project's real .env file
    monkeypatch.setattr(
        "Rxscientist.config.settings.find_dotenv",
    ...
    for key in [
        ...
        "OPENAI_API_KEY",
        "TAVILY_API_KEY",
        "Rxscientist_DEFAULT_MODE",
        "Rxscientist_WORKSPACE_DIR",
        "Rxscientist_UI_BACKEND",
    ]:
...
# =============================================================================
# Test RxscientistConfig dataclass
# =============================================================================


class TestRxscientistConfig:
    ...
    def test_auth_mode_set(self):
        ...
        config = RxscientistConfig(anthropic_auth_mode="oauth")
    ...
    def test_openai_auth_mode_set(self):
        ...
        config = RxscientistConfig(openai_auth_mode="oauth")
...
class TestConfigPaths:
    def test_get_config_dir_with_xdg(self, monkeypatch, tmp_path):
        ...
        assert config_dir == tmp_path / "Rxscientist"

    def test_get_config_dir_default(self, monkeypatch):
        """Test config dir defaults to ~/.config/Rxscientist."""
        monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
        config_dir = get_config_dir()
        assert config_dir == Path.home() / ".config" / "Rxscientist"
...
class TestLoadSaveReset:
    ...
    def test_save_creates_file(self, temp_config_dir, clean_env):
        ...
        config = RxscientistConfig(provider="openai", model="gpt-4o")
        save_config(config)
    ...
    def test_reset_deletes_config_file(self, temp_config_dir, clean_env):
        ...
        config = RxscientistConfig(provider="openai")
        save_config(config)
...
class TestGetSetValues:
    def test_get_config_value(self, temp_config_dir, clean_env):
        ...
        config = RxscientistConfig(model="gpt-4o-mini")
        save_config(config)
...
class TestPriorityChain:
    ...
    def test_file_overrides_defaults(self, temp_config_dir, clean_env):
        ...
        save_config(RxscientistConfig(provider="openai"))
    ...
    def test_env_overrides_file(self, temp_config_dir, monkeypatch):
        ...
        save_config(RxscientistConfig(default_mode="run"))
        monkeypatch.setenv("Rxscientist_DEFAULT_MODE", "daemon")
    ...
    def test_cli_overrides_file(self, temp_config_dir, clean_env):
        ...
        save_config(RxscientistConfig(model="gpt-4o"))
    ...
    def test_env_ui_backend_override(self, temp_config_dir, monkeypatch):
        ...
        save_config(RxscientistConfig(ui_backend="cli"))
        monkeypatch.setenv("Rxscientist_UI_BACKEND", "tui")
        config = get_effective_config()
        assert config.ui_backend == "tui"

    def test_env_log_level_override(self, temp_config_dir, monkeypatch):
        """Log level can be selected via environment variable."""
        save_config(RxscientistConfig(log_level="warning"))
        monkeypatch.setenv("Rxscientist_LOG_LEVEL", "DEBUG")
        config = get_effective_config()
    ...
    def test_env_reasoning_effort_override(self, temp_config_dir, monkeypatch):
        ...
        save_config(RxscientistConfig(reasoning_effort="medium"))
        monkeypatch.setenv("Rxscientist_REASONING_EFFORT", "high")
        config = get_effective_config()
        assert config.reasoning_effort == "high"
    ...
    def test_env_channel_debug_tracing_override(self, temp_config_dir, monkeypatch):
        ...
        save_config(RxscientistConfig(channel_debug_tracing=False))
        monkeypatch.setenv("Rxscientist_CHANNEL_DEBUG_TRACING", "true")
        config = get_effective_config()
    ...
    def test_env_api_key_override(self, temp_config_dir, monkeypatch):
        ...
        save_config(RxscientistConfig(anthropic_api_key="file-key"))
    ...
    def test_env_auth_mode_override(self, temp_config_dir, monkeypatch):
        ...
        save_config(RxscientistConfig(anthropic_auth_mode="api_key"))
        monkeypatch.setenv("Rxscientist_ANTHROPIC_AUTH_MODE", "oauth")
    ...
    def test_env_openai_auth_mode_override(self, temp_config_dir, monkeypatch):
        ...
        save_config(RxscientistConfig(openai_auth_mode="api_key"))
        monkeypatch.setenv("Rxscientist_OPENAI_AUTH_MODE", "oauth")
...
class TestApplyConfigToEnv:
    ...
    def test_does_not_override_existing_env(self, monkeypatch):
        ...
        config = RxscientistConfig(anthropic_api_key="config-key")
        apply_config_to_env(config)
    ...
    def test_empty_config_keys_not_applied(self, clean_env):
        ...
        config = RxscientistConfig()  # All empty
        apply_config_to_env(config)
    ...
    def test_ollama_base_url_applied(self, clean_env, monkeypatch):
        ...
        config = RxscientistConfig(ollama_base_url="http://localhost:11434")
        apply_config_to_env(config)
    ...
    def test_ollama_base_url_not_overridden(self, monkeypatch):
        ...
        config = RxscientistConfig(ollama_base_url="http://new:11434")
        apply_config_to_env(config)
