"""Background update check for Rxscientist.
...
PYPI_URL = "https://pypi.org/pypi/Rxscientist/json"
...
def _installed_version() -> str:
    """Return the currently installed Rxscientist version."""
    from importlib.metadata import version as _pkg_version

    return _pkg_version("Rxscientist")
...
def get_latest_version() -> str | None:
    """Fetch the latest Rxscientist version from PyPI, with 24h caching."""
    # Try cache first
    try:
    ...
    try:
        ...
        req = urllib.request.Request(
            PYPI_URL,
            headers={"User-Agent": "Rxscientist update-check"},
        )
...
def is_update_available() -> tuple[bool, str | None]:
    """Check whether a newer version of Rxscientist is available on PyPI.
