...
Usage:
    python -m Rxscientist.channels.slack.serve --bot-token TOKEN --app-token TOKEN [OPTIONS]

Examples:
    # Allow all senders (default)
    python -m Rxscientist.channels.slack.serve --bot-token xoxb-... --app-token xapp-...

    # Only allow specific senders and channels
    python -m Rxscientist.channels.slack.serve --bot-token xoxb-... --app-token xapp-... --allow U123 --allow-channel C456

    # With proxy, agent and thinking
    python -m Rxscientist.channels.slack.serve --bot-token xoxb-... --app-token xapp-... --proxy http://proxy:8080 --agent --thinking
"""
