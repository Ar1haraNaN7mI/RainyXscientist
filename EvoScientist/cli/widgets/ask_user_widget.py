...
class AskUserWidget(Widget):
    ...
    def compose(self) -> ComposeResult:
        ...
        if total == 1:
            title = ">>> Quick check-in from Rxscientist <<<"
        else:
            title = f">>> Question 1/{total} — Quick check-in from Rxscientist <<<"
    ...
    def _show_question(self, index: int) -> None:
        ...
        if self._title_w:
            if total == 1:
                self._title_w.update(">>> Quick check-in from Rxscientist <<<")
            else:
                self._title_w.update(
                    f">>> Question {index + 1}/{total}"
                    " — Quick check-in from Rxscientist <<<"
                )
