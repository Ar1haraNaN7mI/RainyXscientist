...
def _uv_tool_name() -> str | None:
    ...
    (e.g. ``/home/user/.local/share/uv/tools/Rxscientist`` → ``Rxscientist``).
    """
...
def pip_install_hint() -> str:
    ...
    if _is_uv_tool_env():
        return "uv tool install --reinstall Rxscientist --with"
    if shutil.which("uv"):
...
def _install_with_uv_tool_env(package: str) -> bool:
    """Install *package* into the rxsci uv-tool env via ``--with``.
    ...
    recorded in uv's receipt and survives ``uv tool upgrade Rxscientist``.
    """
...
def install_library(package: str) -> bool:
    ...
    1. **uv tool env**: ``uv tool install Rxscientist --with <pkg>`` —
       package lands in rxsci's isolated env and is importable from it.
...
def install_cli_tool(package: str, *, verify_command: str) -> bool:
    ...
    *verify_command* is the CLI name expected to appear after step 2; if
    ...
    # rxsci's own bin dir (next to sys.executable), not in `uv tool dir
...
def fetch_marketplace_index(
    ...
    entries: list[MCPServerEntry] = []
    with tempfile.TemporaryDirectory(prefix="Rxscientist-mcp-browse-") as tmp:
    ...
    if entry.env_key:
        if entry.env_optional:
            ...
            if not os.environ.get(entry.env_key):
                print_fn(
                    f"  Set it before running Rxscientist: export {entry.env_key}=...",
                    "dim",
                )
