...
```bash
rxsci mcp add <name> <command-or-url> [args...] \
  [--transport <transport>] \
  [--tools <tool1,tool2,...>] \
...
| Command | Description |
|---------|-------------|
| `rxsci mcp` | List configured servers |
| `rxsci mcp list` | List configured servers |
| `rxsci mcp config` | Show detailed config for all servers |
| `rxsci mcp config <name>` | Show detailed config for one server |
| `rxsci mcp add ...` | Add a server |
| `rxsci mcp edit ...` | Edit an existing server |
| `rxsci mcp remove <name>` | Remove a server |
...
rxsci mcp add docs-langchain https://docs.langchain.com/mcp

# SSE endpoint (explicit transport override)
rxsci mcp add research-sse https://example.com/sse --transport sse

# With tool routing to sub-agent + env var reference
rxsci mcp add brave-search npx --env-ref BRAVE_API_KEY -e research-agent -- -y @modelcontextprotocol/server-brave-search

# With tool allowlist (glob wildcards)
rxsci mcp add fs npx -t "read_*,write_*" -- -y @modelcontextprotocol/server-filesystem /workspace

# Context7 routed to multiple agents
rxsci mcp add context7 npx -e main,research-agent,code-agent -- -y @upstash/context7-mcp@latest
```
...
rxsci mcp add filesystem npx -- -y @modelcontextprotocol/server-filesystem /tmp

# http transport (auto-detected from URL)
rxsci mcp add brave-search http://localhost:8080/mcp -H "Authorization:Bearer ${BRAVE_API_KEY}"

# sse transport, routed to a specific agent
rxsci mcp add my-sse http://localhost:9090/sse --transport sse -e research-agent

# With tool allowlist (supports glob wildcards)
rxsci mcp add fs npx -- -y @modelcontextprotocol/server-filesystem /tmp -t "read_*,write_*"
```
...
```bash
# Change routing
rxsci mcp edit filesystem --expose-to main,code-agent

# Set a tool allowlist (supports glob wildcards)
rxsci mcp edit filesystem --tools "read_*,write_*"

# Clear a tool allowlist (pass all tools)
rxsci mcp edit filesystem --tools none

# Change URL
rxsci mcp edit my-api --url http://new-host:9090/mcp
```
...
> Path: `~/.config/Rxscientist/mcp.yaml` (or `$XDG_CONFIG_HOME/Rxscientist/mcp.yaml`)
...
1. On agent startup (`/new` or `create_cli_agent()`), reads `~/.config/Rxscientist/mcp.yaml`
...
Auth header/env becomes empty — ensure the variable exists in your environment before launching Rxscientist. `${VAR}` interpolation happens at runtime; missing vars are replaced with empty strings and logged as warnings.
...
- Confirm endpoint/command is healthy outside Rxscientist
